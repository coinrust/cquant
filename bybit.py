...
class ServerTime:
    ...
    def __new__(timeSecond: int, timeNano: int) -> ServerTime:
...
class ExchangeInfo:
    ...
    def __new__(symbol: str, tick_size: float, step_size: float) -> ExchangeInfo:
        return (symbol, tick_size, step_size)
    
    def __repr__(self) -> str:
        return f"<ExchangeInfo: symbol={self.symbol}, tick_size={self.tick_size}, step_size={self.step_size}>"
...
class KlineItem:
    ...
    def __repr__(self) -> str:
        return f"<KlineItem: timestamp={self.timestamp}, open={self.open}, high={self.high}, low={self.low}, close={self.close}, volume={self.volume}, turnover={self.turnover}, confirm={self.confirm}>"
...
class OrderBookItem:
    ...
    def __new__(_price: float, _qty: float) -> OrderBookItem:
...
class OrderBook:
    ...
    def __new__(_asks: List[OrderBookItem], _bids: List[OrderBookItem]) -> OrderBook:
...
class OrderResponse:
    ...
    def __new__() -> OrderResponse:
        return OrderResponse()
    
    def __repr__(self) -> str:
        return f"<OrderResponse: order_id={self.order_id}, order_link_id={self.order_link_id}>"
...
class BalanceInfo:
    ...
    def __new__() -> BalanceInfo:
        return BalanceInfo()
    
    def __repr__(self) -> str:
        return f"<BalanceInfo: coin_name={self.coin_name} equity={self.equity}, available_to_withdraw={self.available_to_withdraw}, wallet_balance={self.wallet_balance}, total_order_im={self.total_order_im}, total_position_im={self.total_position_im}>"
...
class OrderInfo:
    ...
    def __new__() -> OrderInfo:
        return OrderInfo()
    
    def __repr__(self) -> str:
        return f"<OrderInfo: position_idx={self.position_idx}, order_id={self.order_id}, symbol={self.symbol}, side={self.side}, type={self.type}, price={self.price}, qty={self.qty}, cum_exec_qty={self.cum_exec_qty}, status={self.status}, created_time={self.created_time}, updated_time={self.updated_time}, avg_price={self.avg_price}, cum_exec_fee={self.cum_exec_fee}, time_in_force={self.time_in_force}, reduce_only={self.reduce_only}, order_link_id={self.order_link_id}>"
