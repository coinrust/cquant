...
from bybit import ServerTime, ExchangeInfo, KlineItem, OrderBookItem, OrderBook, \
    PositionInfo, OrderResponse, BalanceInfo, OrderInfo


@tuple
class ct_http_response:
    status: int
    data: str
    
    def __new__(status: int, data: str) -> ct_http_response:
        return ct_http_response(status, data)
...
from C import LIBRARY.seq_bybit_client_do_get(cobj, cobj, cobj, bool, Ptr[ct_http_response]) -> int
# from C import LIBRARY.seq_bybit_client_do_post(cobj, cobj, cobj, bool) -> cobj
from C import LIBRARY.seq_bybit_client_do_post(cobj, cobj, cobj, bool, Ptr[ct_http_response]) -> int
...
class BybitClient:
    ...
    def fetch_public_time(self) -> ServerTime:
        ...
        with yyjson_doc(ret.data) as doc:
            root = doc.root()
